...
attr_accessor :initial_price, :dividend_yield, :interest_rate
...
def spot(time)
  @initial_price*Math.exp((@interest_rate - @dividend_yield)*time)
end
...
#we calculate put_price using the call-put parity formula
def put_price(strike, time)
  call_price(strike, time) - Math.exp(-@interest_rate * time) * (spot(time) - strike)
end


end

p = Asset.new(10, 2, 3, 2)
puts p.call_price(50, 3)
